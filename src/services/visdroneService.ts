...
class VisDroneService {
  ...
  async getNewsById(id: string): Promise<NewsItem | null> {
    ...
    // Fallback: 从本地缓存查找
    try {
      const cachedNews = await localDatabase.get('news');
      if (cachedNews && Array.isArray(cachedNews)) {
        const found = cachedNews.find((n: NewsItem) => n.id === id);
        if (found) return found;
      }
    } catch (e) {
      console.warn('Failed to fetch news from cache:', e);
    }
